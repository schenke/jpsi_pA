...
double singlet(const double p, const double phip, const double k1, const double phik1,
    ...
    double lt = k - p/2. + k1/2.;
    double lt2 = k*k + p*p/4. + k1*k1/4 - k*p*cospk + k*k1*cosk1k - p*k1*cospk1/2;
    // l'_T = k'_T - p_T/2 + k_1T/2
    double lprimet = kprime - p/2 + k1/2;
    double lprimet2 = kprime*kprime + p*p/4. + k1*k1/4 - kprime*p*cospkprime + kprime*k1*cosk1kprime- p*k1*cospk1/2;

    return k1*k1*kappa2*(1./(lt2+kappa2/4.)-1./(lprimet2+kappa2/4.))/(12.*m);

}

double octet(const double p, const double phip, const double k1, const double phik1,
               const double kprime, const double phikprime, const double k, const double phik, 
               const double m){
    ...
    double pdotk1 = 2*p*k1*cospk1;
    double k1dotl = k1*k*cosk1k - k1*p*cospk1/2 + k1*k1/2;
    double pdotl = p*k*cospk - p*p/2 + p*k1*cospk1/2;

    double octet_one = 2*(k1*k1*lt2-kdotl*kdotl)/(m*pow(lt2+kappa2/4,2));

    double octet_two_a = 2*k1*k1*(pow(p*p+k1*k1-pdotk1,2)+4*m*m)/(3*m*m*(p*p+4*m*m));
    double octet_two_b = -4*k1*k1*(pow(p*p+k1*k1-pdotk1,2)+pdotk1+4*m*m)/(3*m*(lt2+kappa2/4)*(p*p+4*m*m));
    double octet_two_c = k1*k1*kappa2/(6*m*pow(lt2+kappa2/4,2));

    double octet_two = octet_two_a+octet_two_b+octet_two_c;
    ...
    double octet_three = octet_three_a+octet_three_b+octet_three_c+octet_three_d;
    return octet_one+octet_two+octet_three;
